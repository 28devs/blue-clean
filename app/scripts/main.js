...
function setasideMenuHeight() {
  let height = u('.aside').size().height - u('.aside__account').size().height - u('.aside__search').size().height - u('.aside__nav-fixed').size().height;
...
function showModal(type = 'default') {
  ...
  let content = document.getElementById('modal-' + type).innerHTML;
  modal.setContent(content);
...
u('[data-tippy-template]').each(function (elem) {
  let id = u(elem).data('tippy-template');
  let content = u('#' + id).html();
  console.log(content)
